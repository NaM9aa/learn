...
var geojs = {
    ...
    intersectWithTriangle: function (lx1, ly1, lx2, ly2, tx1, ty1, tx2, ty2, tx3, ty3) {
        this.intersect(x1, y1, x2, y2, x3, y3, x4, y4);
        var cx = this.intersect.x;
        //var cx = (ly1 - y1 - lx1 * (ly2 - ly1) / (lx2 - lx1) + x1 * (y2 - y1) / (x2 - x1)) / ((y2 - y1) / (x2 - x1) - (ly2 - ly1) / (lx2 - lx1));
        if (){
            document.getElementById("a").innerHTML = x1 + ' , ' + ay; 
        }
